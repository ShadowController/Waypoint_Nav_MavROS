...
# gazebo local frame (x-front, y-left)
    ...
    def compute_waypoints(self):
            ...
            wp_g = [0]*4
            wp_g[0] = self.xy2latlon(wp0, origin)
            wp_g[1] = self.xy2latlon(wp1, origin)
            wp_g[2] = self.xy2latlon(wp2,origin)
            wp_g[3] = self.xy2latlon(wp3,origin)

            for i in range(len(self.wps11)):
                self.wps11[i]=WP_Parameters()
            ...
            for j in range(1, len(self.wps11)):
                ...
                self.wps11[j].x_lat = wp_g[j-1][0]
                self.wps11[j].y_long = wp_g[j-1][1]
                self.wps11[j].z_alt = wp_g[j-1][2]
...
if __name__ == '__main__':
    ...
    if (True):		
        ...
        mav.wpPush(mav.compute_waypoints())
        mav.set_mode("AUTO")	
        mav.toggle_arm(0)
